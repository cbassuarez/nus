...
use cef::{args::Args, *};
use std::{cell::RefCell, process::ExitCode, sync::Arc, time::Duration};
    ...
    fn window_event(&mut self, event_loop: &ActiveEventLoop, _id: WindowId, event: WindowEvent) {
        ...
        match event {
            ...
            WindowEvent::RedrawRequested => {
                FRAMES.with(|f| {
                    let (count, t0) = &mut *f.borrow_mut();
                    *count += 1;
                    if *count % 120 == 0 {
                        let dt = t0.elapsed().as_secs_f64();
                        tracing::info!("{:.1} fps ({} frames)", 120.0 / dt, *count);
...
fn main() -> std::process::ExitCode {
    ...
    let ret = loop {
        ...
        // The upstream example sleeps ~60ms here; we let present() block on vsync instead.
    };
    cef::shutdown();
    ret
}
...
thread_local! {
    static FRAMES: RefCell<(u64, std::time::Instant)> =
        RefCell::new((0, std::time::Instant::now()));
}
