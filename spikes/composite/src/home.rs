...
//! `home` in the palette brings it back. Drawn from the tokens, like every
//! other surface: paper, ink, one signal.
...
impl HomePane {
    pub fn new() -> HomePane {
        HomePane { rect: Rect::new(0.0, 0.0, 1.0, 1.0), input: String::new(), sel: 0, hits: Vec::new(), since: Instant::now() }
    }
}
...
impl App {
    ...
    /// list of places to go when nothing is.
    fn home_rows(&self, input: &str) -> Vec<PaletteRow> {
        ...
        rows.truncate(if input.trim().is_empty() { 7 } else { 9 });
        rows
    }
    ...
    pub(crate) fn draw_home(&mut self, scene: &mut Scene, p: &mut HomePane, focused: bool) {
        ...
        scene.rect(r, paper);
        // The wordmark, small, where the pane begins.
        ...
        let px = self.px(20.0);
        let mono = Style { font: self.f.ui, px, color: ink, tracking: 0.0 };
        let line_w = (r.w * 0.62).max(self.px(320.0)).min(r.w - self.px(56.0));
        let x0 = r.x + (r.w - line_w) / 2.0;
        let y0 = r.y + r.h * 0.34;
        let caret_w = self.fonts.draw(scene, Style { color: self.surface.signal, ..mono }, x0, y0, "»") + self.px(12.0);
        ...
        if focused {
            ...
            if on {
                scene.rect(Rect::new(x0 + caret_w + tw + self.px(2.0), y0 - px * 0.78, px * 0.5, px * 0.95), ink);
            }
            self.dirty = true;
        }
        scene.hline(x0, y0 + self.px(12.0), line_w, self.px(m::HAIRLINE), fade(ink, 0.45));
        // Rows beneath: the palette's, for what is typed.
        let rows = self.home_rows(&p.input);
        p.hits.clear();
        ...
        for (k, row) in rows.iter().enumerate() {
            ...
            if k + 1 == sel {
                scene.rect(Rect::new(x0 - self.px(10.0), y + self.px(6.0), self.px(2.0), row_h - self.px(12.0)), self.surface.signal);
            }
            ...
            self.fonts.draw(scene, Style { color: if hot { ink } else { fade(ink, 0.75) }, ..label }, x0 + num_w, base, &text);
            p.hits.push((rr, k));
            y += row_h;
        }
        // One dim line at the foot, the only words on the page.
        let foot = if sel > 0 { "enter · this row" } else if p.input.is_empty() { "enter · a shell   ·   a url · a page   ·   a command · a shell running it   ·   ↓ the rows" } else if is_url(&p.input) { "enter · this page" } else { "enter · a shell running this" };
        let fw = self.fonts.measure(dim, foot);
        self.fonts.draw(scene, dim, r.x + (r.w - fw) / 2.0, r.bottom() - self.px(26.0), foot);
    }
}
