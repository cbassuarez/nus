...
pub struct Splash {
    ...
    pub fade: Anim,
    pub leaving: bool,
    pub(crate) hyperdrive: Option<crate::hyperdrive::Art>,
}
...
impl Splash {
    pub fn new() -> Splash {
        Splash { arrival: false, started: crate::clock::now(), begun: false, tex: None, fade: Anim::at(1.0), leaving: false, hyperdrive: None }
    }
}
...
impl App {
    pub(crate) fn finish_arrival(&mut self) {
        if self.splash.as_ref().is_some_and(|s|s.arrival) {
            if let Err(e)=crate::store::write_atomic(std::path::Path::new("profile/arrival-seen"),b"1") {tracing::warn!("Could not remember first arrival: {e}");}
        }
    }
}
