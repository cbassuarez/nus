...
p=run('reading',f'tab {article.as_uri()}\nwait 1300\nsavereading\nwait 1800\nlibrary\nwait 350\nreadingassert 1 1000\nshot library\nreadingopen A place\nwait 250\nreadingbounds\nshot reader\nlibraryclick More\nwait 100\nassertpalette open\nassertpromptfirst Open original\nshot reading-options\nkey Escape\nreadingscroll 1800\nwait 1000\nreadingprogress 0.05\nshot reader-resume\nreadingback',base)
entries=list((p/'library').glob('*.json'));assert len(entries)==1
e=json.loads(entries[0].read_text());assert e['snapshot'];assert (p/'library/objects'/f"{e['snapshot']}.article").is_file()
article.unlink()
run('reading','library\nwait 400\nreadingopen A place\nwait 200\nreadingassert 1 1000\nreadingprogress 0.05\nshot offline-resume\nkey cmd+f\ntype breathe\nkey Enter\nshot reader-find\nkey Escape\nkey cmd+plus\nwait 150\nreadingbounds\nshot reader-larger\nkey Escape\nreadingbounds\nshot library-return')
