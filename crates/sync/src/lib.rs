...
fn library_payload(rel: &str) -> bool {
    let (name, length) = if let Some(name) = rel
        .strip_prefix("library/objects/")
        .and_then(|s| s.strip_suffix(".article"))
    {
        (name, 64)
    } else if let Some(name) = rel.strip_prefix("library/").and_then(|s| {
        s.strip_suffix(".json")
            .or_else(|| s.strip_suffix(".article"))
    }) {
        (name, 32)
    } else {
        return false;
    ...
    for part in [&root, &path] {
        if std::fs::symlink_metadata(part).is_ok_and(|m| m.file_type().is_symlink()) {
            return Err(std::io::Error::other(
                "Library paths must not traverse symbolic links",
            ));
        }
    }
...
fn replace_library(path: &Path, bytes: &[u8], written: u64) -> std::io::Result<()> {
    ...
    let parent = path
        .parent()
        .ok_or_else(|| std::io::Error::other("Missing library directory"))?;
            ...
            for directory in ["library", "library/objects"] {
                if let Ok(rd) = std::fs::read_dir(profile.join(directory)) {
                    for e in rd.flatten() {
                        ...
                        if valid && e.file_type().is_ok_and(|t| t.is_file()) {
                            v.push(format!("{directory}/{name}"));
                        }
                    }
                }
            }
...
fn local_manifest_with_library(
    profile: &Path,
    device: &str,
    session: bool,
    library: bool,
...
pub fn exchange(
    ...
    let library_guard = match lock_library(profile) {
        Ok(lock) => Some(lock),
        Err(e) => {
            rep.errors.push(format!("Reading library deferred: {e}"));
            None
        }
    };
    ...
    for (ci, c) in carriers.iter().enumerate() {
        for m in c.manifests(key) {
            ...
            for (rel, e) in &m.files {
                if library_path(rel) && (!library || !library_payload(rel)) {
                    continue;
                }
                let newer = best
                    .get(rel)
    ...
    for c in carriers {
        ...
        if !library {
            published
                .files
                .extend(theirs.into_iter().filter(|(rel, _)| library_payload(rel)));
        }
...
"
    ...
    fn reading_text_and_position_follow_the_encrypted_carrier() {
        ...
        let a = base.join("a");
        let b = base.join("b");
        std::fs::create_dir_all(a.join("library")).unwrap();
        std::fs::create_dir_all(&b).unwrap();
        std::fs::write(
            a.join("library/0123456789abcdef0123456789abcdef.json"),
            br#"{"progress":0.0,"title":"A private article"}"#,
        )
        .unwrap();
        std::fs::write(
            a.join("library/0123456789abcdef0123456789abcdef.article"),
            b"Private offline article text",
        )
        .unwrap();
        let carrier = Folder {
        ...
        let key = new_key();
        assert!(exchange(&a, "a", &key, false, &[&carrier])
            .errors
            .is_empty());
        let report = exchange(&b, "b", &key, false, &[&carrier]);
        assert!(report.errors.is_empty());
        assert_eq!(
            std::fs::read(b.join("library/0123456789abcdef0123456789abcdef.article")).unwrap(),
            b"Private offline article text"
        );
        let progress = br#"{"progress":0.65,"title":"A private article"}"#;
        let path = b.join("library/0123456789abcdef0123456789abcdef.json");
        std::fs::write(&path, progress).unwrap();
        std::fs::File::options()
            .write(true)
            .open(&path)
            .unwrap()
            ...
            .unwrap();
        assert!(exchange(&b, "b", &key, false, &[&carrier])
            .errors
            .is_empty());
        assert!(exchange(&a, "a", &key, false, &[&carrier])
            .errors
            .is_empty());
        assert_eq!(
            std::fs::read(a.join("library/0123456789abcdef0123456789abcdef.json")).unwrap(),
            progress
        );
    ...
    fn reading_objects_sync_without_writer_locks_or_temporary_files() {
        ...
        let a = base.join("a");
        let b = base.join("b");
        std::fs::create_dir_all(a.join("library/objects")).unwrap();
        std::fs::create_dir_all(&b).unwrap();
        let keyname = "0123456789abcdef0123456789abcdef";
        let object = "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef";
        let metadata = format!("library/{keyname}.json");
        let snapshot = format!("library/objects/{object}.article");
        std::fs::write(
            a.join(&metadata),
            format!(r#"{{"snapshot":"{object}","progress":0.6}}"#),
        )
        .unwrap();
        ...
        for local in [
            "library/.writer.lock",
            "library/.tmp123",
            "library/objects/.tmp456",
            "library/objects/not-a-hash.article",
        ] {
            std::fs::write(a.join(local), b"local only").unwrap();
        }
        assert_eq!(
            files_to_sync(&a, false),
            vec![metadata.clone(), snapshot.clone()]
        );
        let folder = Folder {
            root: base.join("carrier"),
        };
        let key = new_key();
        assert!(exchange(&a, "first", &key, false, &[&folder])
            .errors
            .is_empty());
        assert!(exchange(&b, "second", &key, false, &[&folder])
            .errors
            .is_empty());
        assert_eq!(std::fs::read(b.join(snapshot)).unwrap(), b"Offline article");
        assert_eq!(
            std::fs::read(a.join(&metadata)).unwrap(),
            std::fs::read(b.join(metadata)).unwrap()
        );
        assert!(b.join("library/.writer.lock").exists());
        assert!(!files_to_sync(&b, false)
            .iter()
            .any(|p| p.ends_with(".writer.lock")));
        std::fs::remove_dir_all(base).unwrap();
    }
    ...
    fn busy_reading_writer_defers_pull_and_push_without_losing_advertised_files() {
        let base =
            std::env::temp_dir().join(format!("nus-reading-busy-{}-{}", std::process::id(), now()));
        let a = base.join("a");
        let b = base.join("b");
        std::fs::create_dir_all(a.join("library")).unwrap();
        std::fs::create_dir_all(b.join("library")).unwrap();
        ...
        assert_eq!(
            std::fs::read(b.join(format!("{rel}.b.lost"))).unwrap(),
            b"local edit in progress"
        );
        std::fs::remove_dir_all(base).unwrap();
    }
    ...
    fn old_manifests_cannot_replace_library_lock_or_temporary_state() {
        ...
        for rel in [
            ...
            "library/.tmp123",
            "library/objects/.tmp456",
            "library/0123456789abcdef0123456789abcdef.json.old.lost",
        ] {
            folder
                .write(&key, "old", rel, b"old process state")
                .unwrap();
            manifest.files.insert(
                rel.into(),
                Entry {
                    hash: blake3::hash(b"old process state").to_hex().to_string(),
                    written: now() + 10,
                    size: 17,
                },
            );
        ...
        let guard = lock_library(&profile).unwrap();
        drop(guard);
        #[cfg(unix)]
        let inode = {
            use std::os::unix::fs::MetadataExt;
            std::fs::metadata(profile.join("library/.writer.lock"))
                .unwrap()
                .ino()
        };
        ...
        assert_eq!(
            std::fs::read(profile.join("library/.writer.lock")).unwrap(),
            b""
        );
        #[cfg(unix)]
        {
